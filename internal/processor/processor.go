...
func (p *DealProcessor) processNewDeal(ctx context.Context, dealToSave *models.DealInfo, scrapedDuplicates []models.DealInfo, newDeals *[]models.DealInfo, subs []models.Subscription, tracker *metrics.Tracker) error {
	...
	// Initialize rank tracking. RFD heat alone is not enough for warm/hot:
	// those subscription tiers should only promote discount-backed deals.
	p.applyRFDWarmHotState(dealToSave)
...
func (p *DealProcessor) processExistingDeal(ctx context.Context, existing *models.DealInfo, scrapedDuplicates []models.DealInfo, updatedDeals *[]models.DealInfo, subs []models.Subscription) error {
	...
	// Update historical rank tracking using aggregated stats and discount evidence.
	p.applyRFDWarmHotState(existing)
...
func (p *DealProcessor) isDealEligibleForSubscription(deal models.DealInfo, sub models.Subscription) bool {
	...
	isWarm := discountBacked && (deal.HasBeenWarm || p.notifier.IsWarm(deal))
	isHot := discountBacked && (deal.HasBeenHot || p.notifier.IsHot(deal))
...
func (p *DealProcessor) applyRFDWarmHotState(deal *models.DealInfo) {
	...
	if p.notifier.IsWarm(*deal) {
		deal.HasBeenWarm = true
	}
	if p.notifier.IsHot(*deal) {
