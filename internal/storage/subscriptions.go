...
func (c *Client) SaveSubscription(ctx context.Context, sub models.Subscription) error {
	...
	docRef := c.client.Collection(subscriptionsCollection).Doc(sub.GuildID)
	_, err := docRef.Set(ctx, sub)
	if err != nil {
		return fmt.Errorf("failed to save subscription for guild %s: %w", sub.GuildID, err)
	}

	return nil
}

// RemoveSubscription removes a guild's subscription from Firestore.
func (c *Client) RemoveSubscription(ctx context.Context, guildID string) error {
	if _, ok := ctx.Deadline(); !ok {
		var cancel context.CancelFunc
		ctx, cancel = context.WithTimeout(ctx, DefaultTimeout)
		defer cancel()
	...
	docRef := c.client.Collection(subscriptionsCollection).Doc(guildID)
	_, err := docRef.Delete(ctx)
	if err != nil {
		return fmt.Errorf("failed to remove subscription for guild %s: %w", guildID, err)
	}
