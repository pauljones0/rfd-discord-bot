...
var (
	...
	priceDetailsNegativeDiscountRe  = regexp.MustCompile(`(?i)((?:seller\s+|store\s+)?(?:coupon|coupons?|promo(?:tion)?|discounts?|savings)[^$]{0,90}?[-−]\s*(?:c\s*\$|ca\s*\$|\$)\s*([0-9][0-9,]*(?:\.[0-9]{1,2})?))`)
	priceDetailsCouponLabelAmountRe = regexp.MustCompile(`(?i)((?:seller\s+|store\s+)?(?:coupon|coupons?|promo(?:tion)?|discounts?)[^$]{0,90}?(?:c\s*\$|ca\s*\$|\$)\s*([0-9][0-9,]*(?:\.[0-9]{1,2})?))`)
